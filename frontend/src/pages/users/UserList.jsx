...
const UserList = ({ role: forcedRole }) => {
  ...
  const getPageTitle = () => {
    if (forcedRole === "student") return "Student Directory";
  ...
  const getPageDesc = () => {
    if (forcedRole === "student")
      return "Manage enrollment details, academic status, and records for students.";
    ...
    return "Manage profiles for students, faculty, and administrative staff.";
  };
  ...
  const handleExport = async () => {
    try {
      const response = await api.get("/admission/export", {
        params: {
          year: roleFilter === "student" ? deptFilter : null, // example of using filters
          department_id: deptFilter,
        },
      });
  ...
  // Permission Logic
  const canCreate = (() => {
    // robust role slug resolution
    let roleSlug = currentUser?.role_data?.slug;

    // If no slug on user object, try finding it via role_id from the roles list
    ...
    if (roleSlug.includes("hr")) {
      // HR can create employees, but not students
      return forcedRole !== "student";
    }
    if (roleSlug === "admission_admin") {
      // Admission Admin can create students and admission staff
      return forcedRole === "student" || forcedRole === "staff";
    }
    if (roleSlug === "admission_staff") {
      // Admission Staff can create only students
      return forcedRole === "student";
    }
    return false;
  })();
  ...
  const canManageUser = (targetUser) => {
    // robust role slug resolution for current user
    ...
    // Admission Admin: Manage Students and Admission Staff
    if (myRoleSlug === "admission_admin") {
      return targetRole === "student" || targetRole === "admission_staff";
    }

    // Admission Staff: Manage Students only
    if (myRoleSlug === "admission_staff") {
      return targetRole === "student";
    }

    // HOD: Manage Dept Users (Student, Faculty, Staff)
    if (myRoleSlug === "hod") {
      const allowedTargets = ["student", "faculty", "staff"];
    ...
    // Dept Staff: Manage Students of OWN Dept only
    if (targetRole === "student") {
      // If I am staff (not admin/hr/admission/hod)
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-[10px] font-mono text-gray-400 uppercase tracking-tighter">
                          ID: {user.student_id || user.employee_id || "N/A"}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
      ...
      <BulkCommunicationModal
        ...
        filters={{ role: roleFilter, department: deptFilter }}
      />
    </div>
  );
};
