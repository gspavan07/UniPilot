...
exports.getAdmissionAnalytics = async (req, res) => {
    try {
        ...
        const activeBatchYear = activeConfig?.batch_year || new Date().getFullYear();

        // Determine batch filter
        const batchWhere = batch && batch !== "all" ? { batch_year: parseInt(batch) } : {};
        ...
        const batchGrowthWithPercentage = batchGrowth.map((batch, index) => {
            ...
            if (index > 0) {
                ...
                growth = ((currentCount - previousCount) / previousCount * 100).toFixed(1);
            }
            return {
        ...
        const allBatches = await User.findAll({
            attributes: [[sequelize.fn("DISTINCT", sequelize.col("batch_year")), "batch_year"]],
            where: {
                role: "student",
                batch_year: { [Op.ne]: null },
            },
        ...
        const batchList = batchYears.map((year) =>
        ...
        res.status(200).json({
            ...
            data: {
                kpis: {
                    ...
                    internationalPercentage: totalStudents > 0
