...
export async function calculateEligibility(studentId, cycleId) {
  try {
    ...
    if (cycle.check_fee_clearance) {
      const feeStatus = await feeController.calculateFeeStatus(studentId);
...
export async function getCycleEligibilities(req, res) {
  try {
    ...
    const students = await User.findAll({
      ...
      include: [
        {
          model: "Program", // Include program details for name if possible, or just rely on ID
          as: "program",
        ...
        },
        {
          ...
          required: false, // Include students even if eligibility not yet calculated
        },
      ],
      include: [
        {
