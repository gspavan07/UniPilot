...
import studentDocumentService from "../../admissions/services/studentDocumentService.js";
import salaryService from "../../hr/services/salaryService.js";
import academicLookupService from "../../academics/services/academicLookupService.js";
import { encrypt } from "../../../utils/encryption.js";
import {
  generateStudentId,
  generateGlobalAdmissionNumber,
} from "../../admissions/services/admissionService.js";
...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      ...
      await studentDocumentService.createStudentDocuments({
        userId: user.id,
        files: req.files,
        documentTypes,
      });
    ...
    if (req.body.salary_grade_id) {
      try {
        const structure = await salaryService.createSalaryStructureFromGrade({
          userId: user.id,
          gradeId: req.body.salary_grade_id,
          effectiveFrom: new Date(),
        });
        if (structure) {
          logger.info(
            `Created default Salary Structure for user ${user.id} using grade ${req.body.salary_grade_id}`,
          );
        }
      } catch (err) {
