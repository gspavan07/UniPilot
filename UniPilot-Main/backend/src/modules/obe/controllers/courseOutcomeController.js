...
import { CourseOutcome } from "../models/index.js";

const hydrateOutcomesWithCourses = async (outcomes) => {
    ...
    const courseIds = list.map((outcome) => outcome.course_id).filter(Boolean);
    if (courseIds.length === 0) return;

    const courseMap = await academicLookupService.getCourseMapByIds(courseIds, {
        attributes: ["id", "name", "code"],
    });

    list.forEach((outcome) => {
        const course = courseMap.get(outcome.course_id) || null;
        if (typeof outcome?.setDataValue === "function") {
            outcome.setDataValue("course", course);
        } else {
...
export const getAllCourseOutcomes = async (req, res) => {
    try {
        ...
        await hydrateOutcomesWithCourses(outcomes);
...
export const getCourseOutcomeById = async (req, res) => {
    try {
        ...
        await hydrateOutcomesWithCourses(outcome);
...
export const createCourseOutcome = async (req, res) => {
    try {
        ...
        const course = await academicLookupService.getCourseById(course_id, {
            attributes: ["id"],
        });
        if (!course) {
            return res.status(404).json({
