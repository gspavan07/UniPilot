...
import { ProgramOutcome } from "../models/index.js";

const hydrateOutcomesWithPrograms = async (outcomes) => {
    ...
    const programIds = list.map((outcome) => outcome.program_id).filter(Boolean);
    ...
    list.forEach((outcome) => {
        const program = programMap.get(outcome.program_id) || null;
        if (typeof outcome?.setDataValue === "function") {
            outcome.setDataValue("program", program);
        } else {
...
export const getAllProgramOutcomes = async (req, res) => {
    try {
        ...
        await hydrateOutcomesWithPrograms(outcomes);
...
export const getProgramOutcomeById = async (req, res) => {
    try {
        ...
        await hydrateOutcomesWithPrograms(outcome);
