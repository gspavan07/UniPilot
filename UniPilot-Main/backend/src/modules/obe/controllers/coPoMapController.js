...
import { CoPoMap, CourseOutcome, ProgramOutcome } from "../models/index.js";
import academicLookupService from "../../academics/services/academicLookupService.js";

const hydrateMappingsWithAcademics = async (mappings) => {
    ...
    const courseOutcomes = list
        .map((mapping) => mapping.courseOutcome)
        .filter(Boolean);
    const programOutcomes = list
        .map((mapping) => mapping.programOutcome)
        .filter(Boolean);

    const courseIds = courseOutcomes.map((co) => co.course_id).filter(Boolean);
    const programIds = programOutcomes
        .map((po) => po.program_id)
        .filter(Boolean);

    const [courseMap, programMap] = await Promise.all([
        academicLookupService.getCourseMapByIds(courseIds, {
    ...
    courseOutcomes.forEach((co) => {
        const course = courseMap.get(co.course_id) || null;
        if (typeof co?.setDataValue === "function") {
            co.setDataValue("course", course);
        } else {
            co.course = course;
        }
    });
    ...
    programOutcomes.forEach((po) => {
        const program = programMap.get(po.program_id) || null;
        if (typeof po?.setDataValue === "function") {
            po.setDataValue("program", program);
        } else {
            po.program = program;
        }
    });
...
export const getCoPoMappings = async (req, res) => {
    try {
        ...
        await hydrateMappingsWithAcademics(mappings);
