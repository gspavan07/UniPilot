...
const hydrateAllocationsWithFeeStructures = async (allocations) => {
  const list = Array.isArray(allocations)
    ? allocations.filter(Boolean)
    : allocations
      ? [allocations]
      : [];
  ...
  list.forEach((allocation) => {
    if (allocation.rent_fee_id) feeIds.push(allocation.rent_fee_id);
    if (allocation.mess_fee_id) feeIds.push(allocation.mess_fee_id);
  });

  if (feeIds.length === 0) return;

  const feeMap = await feeLedgerService.getFeeStructureMapByIds(feeIds, {
    attributes: ["id", "amount", "is_active"],
  });

  list.forEach((allocation) => {
    const rentFee = feeMap.get(allocation.rent_fee_id) || null;
    const messFee = feeMap.get(allocation.mess_fee_id) || null;

    if (typeof allocation?.setDataValue === "function") {
      allocation.setDataValue("rent_fee", rentFee);
      allocation.setDataValue("mess_fee", messFee);
    } else {
      allocation.rent_fee = rentFee;
      allocation.mess_fee = messFee;
    }
  });
};

const hydrateAllocationsWithAcademics = async (allocations) => {
  const list = Array.isArray(allocations)
    ? allocations.filter(Boolean)
    : allocations
      ? [allocations]
...
export const allocateStudent = async (req, res) => {
  ...
  try {
    ...
    const rentCategory = await feeLedgerService.getOrCreateCategory(
      {
        name: "Hostel Rent",
        defaults: {
    ...
    const messCategory = await feeLedgerService.getOrCreateCategory(
      {
        name: "Hostel Mess",
        defaults: {
    ...
    for (let sem = startSem; sem <= endSem; sem++) {
      ...
      const rentCharge = await feeLedgerService.createStudentCharge(
        {
      ...
      const messCharge = await feeLedgerService.createStudentCharge(
        {
          student_id: student_id,
...
export const checkoutStudent = async (req, res) => {
  ...
  try {
    ...
    const hostelCategories = await feeLedgerService.getCategoriesByNames(
      ["Hostel Rent", "Hostel Mess"],
      { attributes: ["id"] },
    );
    ...
    if (checkout_type === "next") {
      ...
      await feeLedgerService.updateFeeStructures(
        { is_active: false },
        {
          where: {
