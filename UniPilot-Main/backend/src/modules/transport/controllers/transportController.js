...
export const createAllocation = async (req, res) => {
  ...
  try {
    ...
    const transportCategory = await feeLedgerService.getOrCreateCategory(
      {
        name: "Transport Fee",
        defaults: {
    ...
    await feeLedgerService.updateStudentCharges(
      ...
      { where: { id: feeCharge.id }, transaction },
    );
...
export const syncSemesterFees = async (req, res) => {
  ...
  try {
    ...
    for (const alloc of activeAllocations) {
      ...
      const existingCharge = await feeLedgerService.findStudentCharge({
        where: {
          category_id: transportCategory.id,
      ...
      if (existingCharge) {
        ...
        if (
          ...
          await feeLedgerService.updateStudentCharges(
            ...
            { where: { id: existingCharge.id }, transaction },
          );
          updatedCount++;
        }
      } else {
        // Create new fee charge entry
        const newCharge = await feeLedgerService.createStudentCharge(
          {
            student_id: alloc.student_id,
            category_id: transportCategory.id,
            charge_type: "transport_fee",
...
export const getDashboardOverview = async (req, res) => {
  try {
    const [
      ...
      feeLedgerService.findStudentCharges({
